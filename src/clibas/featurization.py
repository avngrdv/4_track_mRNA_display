...
def from_matrix_v3(X, alphabet=None, F=None, reshape=True, repad=False):
 ...
 alphabet:     Current representations of X
   ...
   reshape:    Should be flagged if a peptide sequence is to be represented as
    ...
    #TODO: the inference will break it for small datasets!
    if alphabet is None:
    ...
    #TODO: this breaks if arrays are NUM represented: -1 is the pad not the dtype
    #check whether the array is padded
    pad = X.dtype.type()
    _is_padded = pad in X
    
    
    #the only way this works for NUMREP arrays is if alphabet is inferred lol
...
def into_h5(X, y=None, path=None, F=None, alphabet=None, reshape=False, repad=False, chunks=20):
    ...
    import h5py
    #THIS WILL BREAK IS THE DATA IS PADDED BUT YOU WANT ONE HOT
    #TODO: fix, right now a bandaid
    if F is None:
        alphabet = np.asarray(alphabet)
