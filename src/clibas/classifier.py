...
class Classifier(Handler):
    ...
    def _load_h5_test_train_datasets(self, *args):
        ...
        # train_set = tf.data.Dataset.zip((X_train, y_train)).take(self.sample_size)
        # train_set = train_set.batch(self.batch_size).shuffle(self.shuffle_buffer, reshuffle_each_iteration=True).prefetch(tf.data.experimental.AUTOTUNE)
