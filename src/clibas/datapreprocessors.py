...
class DataPreprocessor(Handler):
    ...
    def to_h5(self, reshape=False, alphabet=None, repad=False, chunks=None, return_data=False):
        ...
        def featurize_to_h5(data):
            for sample in data:
                ...
                #TODO: fix the alphabet inference
                from clibas.featurization import FeatureMatrix
                self.F = FeatureMatrix.make(descr='pep_ECFP4', constants=self.constants).F
    
                # self.F = None
                alphabet = np.array(self.constants.aas)
                # print(self.constants.aas)
    ...
    def x_expand_and_shift(self, new_x_dim=None):
        #quick and dirty
        #!!! rewrite properly
        #the arrays have to be NUM REPR
